"""Developer-only CPU checks for Hoodie; does not compile GLSL or run VS Code/WebGL."""
...
meta = json.loads((root / 'hoodie_sdf_128.json').read_text())
im = np.asarray(Image.open(root / 'hoodie_sdf_128_atlas.png').convert('RGBA'))
...
reference = np.fromfile(root / 'hoodie_sdf_128.f32', dtype='<f4').reshape(128,128,128)
roundtrip_error = float(np.abs(volume - reference).max())
assert roundtrip_error < 0.000020

probes = [(p['x'], p['y'], p['r'], p['g']) for p in meta['probes']]
...
def render(angle):
    ...
    hits=np.zeros(len(ro),bool);max_steps=1000
    ...
    col=np.full((len(ro),3),.90);col[hits]=.88*light[:,None];col[capped]=[1,0,1]
...
out=Image.new('RGB',(W*4,H+60),(230,230,230));draw=ImageDraw.Draw(out)
