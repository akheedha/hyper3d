#iChannel0 "file://hoodie_sdf_128_atlas.png"
...
// TIMED COLORS / Hoodie Category / 128-cubed SDF atlas.
// The extension supplies uniforms and wraps mainImage. Do not add #version.
// debugMode: 0=hoodie, 1=atlas + green status strip, 2=distance slice.
...
const vec3 TC_MIN = vec3(-1.02178639, -0.07764339, -1.02293338);
const vec3 TC_MAX = vec3(1.02626739, 1.97041039, 1.02512040);
const float TC_H = 0.016000420153;
const float TC_RANGE = 1.230000;
...
float atlasError(float flipY) {
    ...
    vec2 a = abs(atlasBytes(vec2(37.0, 61.0), flipY) - vec2(207.0, 255.0));
    vec2 b = abs(atlasBytes(vec2(1021.0, 387.0), flipY) - vec2(170.0, 230.0));
    vec2 c = abs(atlasBytes(vec2(1703.0, 911.0), flipY) - vec2(191.0, 155.0));
...
float traceHoodie(vec3 ro, vec3 rd, out float t) {
    float nearT, farT;
    t = 0.0;
    ...
    for (int i = 0; i < MAX_STEPS; ++i) {
        ...
        float d = hoodieSDF(ro + rd * t);
        if (d <= TC_H * 0.004) {
            if (d < 0.0) {
                float leftT = previousT, rightT = t;
                for (int j = 0; j < 10; ++j) {
                    float midT = 0.5 * (leftT + rightT);
                    if (hoodieSDF(ro + rd * midT) < 0.0) rightT = midT;
                    else leftT = midT;
                }
                t = 0.5 * (leftT + rightT);
            }
        ...
        previousT = t;
        // Practical conservative step for this sampled field
        t += max(0.65 * d, TC_H * 0.025);
    }
    return t > farT ? 0.0 : -1.0;
}

vec3 hoodieNormal(vec3 p) {
    float e = TC_H * 0.35;
    vec3 n = vec3(
        hoodieSDF(p + vec3(e,0,0)) - hoodieSDF(p - vec3(e,0,0)),
        hoodieSDF(p + vec3(0,e,0)) - hoodieSDF(p - vec3(0,e,0)),
        hoodieSDF(p + vec3(0,0,e)) - hoodieSDF(p - vec3(0,0,e)));
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    ...
    float t;
    float status = traceHoodie(ro, rd, t);
    vec3 col = vec3(0.90);
    if (status > 0.5) {
        vec3 n = hoodieNormal(ro + rd * t);
